...
public class AuthenticationProcessingFilter extends AbstractAuthenticationProcessingFilter {
    ...
    public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            session.setAttribute(ApplicationConstant.SESSION_ERROR_MSG, e.getMessage());
            operateLog.setOptTagValue("用户登录失败: " + e.getMessage());
            operateLog.setOptManId(0L);
            operateLog.setOptAccount(username);
            operateLog.setOptEnd(end);
            operateLog.setOptSecond(end - start);
        ...
        try {
            ...
            session.setAttribute("officeUrl", cacheUnionSetting.single("wap_url"));// 官网地址

            end = System.currentTimeMillis();
            operateLog.setOptManId(manage.getManId());
            operateLog.setOptAccount(username);
            operateLog.setOptTagValue("用户登录成功");
            operateLog.setOptEnd(end);
            operateLog.setOptSecond(end - start);
            //sysOperateLogService.save(operateLog);
            operateSaveJob.addOperate(operateLog);
