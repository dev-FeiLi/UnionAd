...
public class UnionStatsService {
     ...
     * @return
     */
    public UnionStats totalSum(List<UnionStats> list) {
        UnionStats unionStats = new UnionStats();
        if (list != null) {
            for (UnionStats us : list) {
                unionStats.setPaynum(unionStats.getPaynum() + us.getPaynum());
                unionStats.setDedunum(unionStats.getDedunum() + us.getDedunum());
                unionStats.setSumadvpay(unionStats.getSumadvpay() + us.getSumadvpay());
            }
        }
        return unionStats;
    }
