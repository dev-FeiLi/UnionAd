...
public class AdvController {
    ...
    public ModelAndView advindex(HttpServletRequest request, HttpSession session) {
        ...
        try {
            ...
            // 广告商后台 增加一个广告名称搜索
            List<UnionStats> list2 = new ArrayList<>();
            if (!StringUtils.isEmpty(planname) && list != null && list.size() > 0){
                for (UnionStats us : list) {
                    AdsPlan planTemp = planmap.get(us.getPlanid());
                    if (planTemp == null) continue;
                    if (StringUtils.isEmpty(planTemp.getTitle())) continue;
                    if (planTemp.getTitle().indexOf(planname) != -1) {
                        list2.add(us);
                    }
                }
            }
            ...
            if (null != list && list.size() > 0) {
                ...
                mav.addObject("footer", unionStatsService.totalSum(list));
            }

            if (!StringUtils.isEmpty(planname) && list2.size() > 0) {
                mav.addObject("statlist", list2);
                mav.addObject("footer", unionStatsService.totalSum(list2));
            }

            mav.addObject("starttime", starttime);
            mav.addObject("stoptime", stoptime);
            mav.addObject("planname", planname);
        } catch (Exception e) {
