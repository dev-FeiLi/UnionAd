...
import io.union.log.common.redis.CacheStatHourClickuv;
 ...
 */
@Component
public class StatHourClickuvConsumer {
    final Logger logger = LoggerFactory.getLogger(getClass());

    public StatHourClickuvConsumer(@Value("${spring.kafka.bootstrap-servers}") String hosts, @Autowired StatHourClickuvService statHourClickuvService, @Autowired CacheStatHourClickuv cacheStatHourClickuv,
                                   @Autowired ZooUtils zooutils, @Value("${spring.zookeeper.lock.hour.clickuv}") String rootPath, @Autowired CacheLockPach cacheLockPach) {
        ...
        props.put(ConsumerConfig.GROUP_ID_CONFIG, ApplicationConstant.STAT_HOUR_CLICKUV_COUNT_GROUP);
        props.put(ConsumerConfig.ENABLE_AUTO_COMMIT_CONFIG, "true");
        props.put(ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG, "org.apache.kafka.common.serialization.StringDeserializer");
        props.put(ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG, "org.apache.kafka.common.serialization.StringDeserializer");

        KafkaConsumer<String, String> consumer = new KafkaConsumer<>(props);
        consumer.subscribe(Collections.singletonList(ApplicationConstant.STAT_HOUR_CLICKUV_COUNT_TOPIC));
        ...
        Runnable runnable = () -> {
            while (!closed.get()) {
                try {
                    ...
                    Map<String, StatHourClickuv> tempmap = new HashMap<>();
                    while (queue.size() != 0) {
                        try {
                            ...
                            StatHourClickuv statHourIp = tempmap.get(key);
                            if (null == statHourIp) {
                                statHourIp = new StatHourClickuv();
                                statHourIp.setAddTime(calendar.getTime());
                                statHourIp.setPlanid(planid);
                                statHourIp.setSiteid(siteid);
                                statHourIp.setUid(uid);
                            }
                            StatHourTool<StatHourClickuv> hourIpTool = new StatHourTool<>();
                    ...
                    tempmap.forEach((k, v) -> {
                        ...
                        try {
                            if (lock.acquire(zooutils.TIMEOUT, TimeUnit.SECONDS)) {
                                ...
                                StatHourClickuv statHourIp = cacheStatHourClickuv.single(uid, siteid, planid, today);
                                ...
                                statHourIp = statHourClickuvService.saveOne(statHourIp);
                                cacheStatHourClickuv.save(statHourIp);
                            } else {
