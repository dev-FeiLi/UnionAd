...
import io.union.log.common.redis.CacheStatHourUv;
 ...
 */
@Component
public class StatHourUvConsumer {
    final Logger logger = LoggerFactory.getLogger(getClass());

    public StatHourUvConsumer(@Value("${spring.kafka.bootstrap-servers}") String hosts, @Autowired StatHourUvService statHourUvService, @Autowired CacheStatHourUv cacheStatHourUv,
                              @Autowired ZooUtils zooutils, @Value("${spring.zookeeper.lock.hour.uv}") String rootPath, @Autowired CacheLockPach cacheLockPach) {
        ...
        props.put(ConsumerConfig.GROUP_ID_CONFIG, ApplicationConstant.STAT_HOUR_UV_COUNT_GROUP);
        props.put(ConsumerConfig.ENABLE_AUTO_COMMIT_CONFIG, "true");
        props.put(ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG, "org.apache.kafka.common.serialization.StringDeserializer");
        props.put(ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG, "org.apache.kafka.common.serialization.StringDeserializer");

        KafkaConsumer<String, String> consumer = new KafkaConsumer<>(props);
        consumer.subscribe(Collections.singletonList(ApplicationConstant.STAT_HOUR_UV_COUNT_TOPIC));
        ...
        Runnable runnable = () -> {
            while (!closed.get()) {
                try {
                    ...
                    Map<String, StatHourUv> tempmap = new HashMap<>();
                    while (queue.size() != 0) {
                        try {
                            ...
                            StatHourUv statHourIp = tempmap.get(key);
                            if (null == statHourIp) {
                                statHourIp = new StatHourUv();
                                statHourIp.setAddTime(calendar.getTime());
                                statHourIp.setPlanid(planid);
                                statHourIp.setSiteid(siteid);
                                statHourIp.setUid(uid);
                            }
                            StatHourTool<StatHourUv> hourIpTool = new StatHourTool<>();
                    ...
                    tempmap.forEach((k, v) -> {
                        ...
                        try {
                            if (lock.acquire(zooutils.TIMEOUT, TimeUnit.SECONDS)) {
                                ...
                                StatHourUv statHourIp = cacheStatHourUv.single(uid, siteid, planid, today);
                                ...
                                statHourIp = statHourUvService.saveOne(statHourIp);
                                cacheStatHourUv.save(statHourIp);
                            } else {
