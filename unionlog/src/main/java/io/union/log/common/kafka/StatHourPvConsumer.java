...
import io.union.log.common.redis.CacheStatHourPv;
 ...
 */
@Component
public class StatHourPvConsumer {
    final Logger logger = LoggerFactory.getLogger(getClass());

    public StatHourPvConsumer(@Value("${spring.kafka.bootstrap-servers}") String hosts, @Autowired StatHourPvService statHourPvService, @Autowired CacheStatHourPv cacheStatHourPv,
                              @Autowired ZooUtils zooutils, @Value("${spring.zookeeper.lock.hour.pv}") String rootPath, @Autowired CacheLockPach cacheLockPach) {
        ...
        props.put(ConsumerConfig.GROUP_ID_CONFIG, ApplicationConstant.STAT_HOUR_PV_COUNT_GROUP);
        props.put(ConsumerConfig.ENABLE_AUTO_COMMIT_CONFIG, "true");
        props.put(ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG, "org.apache.kafka.common.serialization.StringDeserializer");
        props.put(ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG, "org.apache.kafka.common.serialization.StringDeserializer");

        KafkaConsumer<String, String> consumer = new KafkaConsumer<>(props);
        consumer.subscribe(Collections.singletonList(ApplicationConstant.STAT_HOUR_PV_COUNT_TOPIC));
        ...
        Runnable runnable = () -> {
            while (!closed.get()) {
                try {
                    ...
                    Map<String, StatHourPv> tempmap = new HashMap<>();
                    while (queue.size() != 0) {
                        try {
                            ...
                            StatHourPv statHourIp = tempmap.get(key);
                            if (null == statHourIp) {
                                statHourIp = new StatHourPv();
                                statHourIp.setAddTime(calendar.getTime());
                                statHourIp.setPlanid(planid);
                                statHourIp.setSiteid(siteid);
                                statHourIp.setUid(uid);
                            }
                            StatHourTool<StatHourPv> hourIpTool = new StatHourTool<>();
                    ...
                    tempmap.forEach((k, v) -> {
                        ...
                        try {
                            if (lock.acquire(zooutils.TIMEOUT, TimeUnit.SECONDS)) {
                                ...
                                StatHourPv statHourIp = cacheStatHourPv.single(uid, siteid, planid, today);
                                ...
                                statHourIp = statHourPvService.saveOne(statHourIp);
                                cacheStatHourPv.save(statHourIp);
                            } else {
