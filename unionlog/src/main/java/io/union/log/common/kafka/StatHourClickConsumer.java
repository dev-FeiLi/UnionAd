...
import io.union.log.common.redis.CacheStatHourClick;
 ...
 */
@Component
public class StatHourClickConsumer {
    final Logger logger = LoggerFactory.getLogger(getClass());

    public StatHourClickConsumer(@Value("${spring.kafka.bootstrap-servers}") String hosts, @Autowired StatHourClickService statHourClickService, @Autowired CacheStatHourClick cacheStatHourClick,
                                 @Autowired ZooUtils zooutils, @Value("${spring.zookeeper.lock.hour.click}") String rootPath, @Autowired CacheLockPach cacheLockPach) {
        ...
        props.put(ConsumerConfig.GROUP_ID_CONFIG, ApplicationConstant.STAT_HOUR_CLICK_COUNT_GROUP);
        props.put(ConsumerConfig.ENABLE_AUTO_COMMIT_CONFIG, "true");
        props.put(ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG, "org.apache.kafka.common.serialization.StringDeserializer");
        props.put(ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG, "org.apache.kafka.common.serialization.StringDeserializer");

        KafkaConsumer<String, String> consumer = new KafkaConsumer<>(props);
        consumer.subscribe(Collections.singletonList(ApplicationConstant.STAT_HOUR_CLICK_COUNT_TOPIC));
        ...
        Runnable runnable = () -> {
            while (!closed.get()) {
                try {
                    ...
                    Map<String, StatHourClick> tempmap = new HashMap<>();
                    while (queue.size() != 0) {
                        try {
                            ...
                            StatHourClick statHourIp = tempmap.get(key);
                            if (null == statHourIp) {
                                statHourIp = new StatHourClick();
                                statHourIp.setAddTime(calendar.getTime());
                                statHourIp.setPlanid(planid);
                                statHourIp.setSiteid(siteid);
                                statHourIp.setUid(uid);
                            }
                            StatHourTool<StatHourClick> hourIpTool = new StatHourTool<>();
                    ...
                    tempmap.forEach((k, v) -> {
                        ...
                        try {
                            if (lock.acquire(zooutils.TIMEOUT, TimeUnit.SECONDS)) {
                                ...
                                StatHourClick statHourIp = cacheStatHourClick.single(uid, siteid, planid, today);
                                ...
                                statHourIp = statHourClickService.saveOne(statHourIp);
                                cacheStatHourClick.save(statHourIp);
                            } else {
