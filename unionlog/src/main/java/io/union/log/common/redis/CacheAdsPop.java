...
public class CacheAdsPop {
    ...
    public void pushpop(String type, Long id, String ip) {
        Jedis jedis = pool.pull();
        try {
            String key = String.format(String.valueOf(RedisKeys.ADS_POP_IP_MAP), type, id), filed = ip;
            ...
            logger.error("pushpop id=" + id + " error: ", e);
        } finally {
            pool.push(jedis);
        }
    }
    ...
    public void pushdiv(String type, Long id, String ip) {
        Jedis jedis = pool.pull();
        try {
            String key = String.format(String.valueOf(RedisKeys.ADS_DIV_IP_MAP), type, id), filed = ip;
            jedis.hset(key, filed, "0");
            ...
            logger.error("pushdiv id=" + id + " error: ", e);
        } finally {
            pool.push(jedis);
        }
    }
