...
import io.union.log.common.redis.CacheStatHourClickip;
 ...
 */
@Component
public class StatHourClickipConsumer {
    final Logger logger = LoggerFactory.getLogger(getClass());

    public StatHourClickipConsumer(@Value("${spring.kafka.bootstrap-servers}") String hosts, @Autowired StatHourClickipService statHourClickipService, @Autowired CacheStatHourClickip cacheStatHourClickip,
                                   @Autowired ZooUtils zooutils, @Value("${spring.zookeeper.lock.hour.clickip}") String rootPath, @Autowired CacheLockPach cacheLockPach) {
        ...
        props.put(ConsumerConfig.GROUP_ID_CONFIG, ApplicationConstant.STAT_HOUR_CLICKIP_COUNT_GROUP);
        props.put(ConsumerConfig.ENABLE_AUTO_COMMIT_CONFIG, "true");
        props.put(ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG, "org.apache.kafka.common.serialization.StringDeserializer");
        props.put(ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG, "org.apache.kafka.common.serialization.StringDeserializer");

        KafkaConsumer<String, String> consumer = new KafkaConsumer<>(props);
        consumer.subscribe(Collections.singletonList(ApplicationConstant.STAT_HOUR_CLICKIP_COUNT_TOPIC));
        ...
        Runnable runnable = () -> {
            while (!closed.get()) {
                try {
                    ...
                    Map<String, StatHourClickip> tempmap = new HashMap<>();
                    while (queue.size() != 0) {
                        try {
                            ...
                            StatHourClickip statHourIp = tempmap.get(key);
                            if (null == statHourIp) {
                                statHourIp = new StatHourClickip();
                                statHourIp.setAddTime(calendar.getTime());
                                statHourIp.setPlanid(planid);
                                statHourIp.setSiteid(siteid);
                                statHourIp.setUid(uid);
                            }
                            StatHourTool<StatHourClickip> hourIpTool = new StatHourTool<>();
                    ...
                    tempmap.forEach((k, v) -> {
                        ...
                        try {
                            if (lock.acquire(zooutils.TIMEOUT, TimeUnit.SECONDS)) {
                                ...
                                StatHourClickip statHourIp = cacheStatHourClickip.single(uid, siteid, planid, today);
                                ...
                                statHourIp = statHourClickipService.saveOne(statHourIp);
                                cacheStatHourClickip.save(statHourIp);
                            } else {
