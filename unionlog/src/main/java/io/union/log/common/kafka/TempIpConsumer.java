...
import io.union.log.service.TempIpService;
 ...
 */
@Component
public class TempIpConsumer {
    final Logger logger = LoggerFactory.getLogger(getClass());

    public TempIpConsumer(@Value("${spring.kafka.bootstrap-servers}") String hosts, @Autowired TempIpService tempIpService) {
        ...
        props.put(ConsumerConfig.GROUP_ID_CONFIG, ApplicationConstant.TEMP_IP_COUNT_GROUP);
        props.put(ConsumerConfig.ENABLE_AUTO_COMMIT_CONFIG, "true");
        props.put(ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG, "org.apache.kafka.common.serialization.StringDeserializer");
        props.put(ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG, "org.apache.kafka.common.serialization.StringDeserializer");

        KafkaConsumer<String, String> consumer = new KafkaConsumer<>(props);
        consumer.subscribe(Collections.singletonList(ApplicationConstant.TEMP_IP_COUNT_TOPIC));
        ...
        Runnable runnable = () -> {
            while (!closed.get()) {
                try {
                    ...
                    Map<String, TempIp> map = new HashMap<>();
                    while (queue.size() != 0) {
                        try {
                    ...
                    List<TempIp> list = new ArrayList<>(map.values());
                    if (list.size() > 0) {
                        tempIpService.save(list);
                        // cacheTempIp.save(list);
                    }
                } catch (Exception e) {
