!(function () {
    ...
    doc.onclick=function (e) {
        ...
        switch (id){
            case '${idname}Gb':
                ...
                break;
            case '${idname}Mc':
                e.target.setAttribute("href", url + "&pos=" + pos);
                var div = e.target.parentNode;
                if (div) div.removeChild(e.target);
                break;
            case '${idname}He':
                e.target.setAttribute("href", url + "&pos=" + pos);
                var div = e.target.parentNode;
    ...
    var LP=setInterval(function () {
        var wc=doc.getElementById("${idname}Wc");
        if (!wc) a.B(_b); else if (wc.style.display==="none"||wc.style.visibility.startsWith("hidden")){doc.body.removeChild(wc);a.B(_b);}
    },100);
    //</c:if>
