!(function () {
    var _elem = "${elemName}",N=navigator, doc = document, win = window, D = screen, hc=eval(${hcontrol})||{}, pos = "0,0", a = {
        A: function (css) {
    ...
    function __C() {
        try {
            ...
            if(mc){this.parentNode.removeChild(mc);}
    ...
    var url = '${clickurl}&cli=' + __P(), height=parseInt(D.width*parseFloat('${height/width}'));
    var _c = ".${idname}Wc {margin:0;padding:0;border:0;} ";
    _c += ".${idname}Gb {position:absolute;right:0;top:0;width:18px;height:18px;z-index:2147483648 !important;display:block;} ";
    _c += ".${idname}Gb img{width:18px; height:18px;} ";//<c:if test='${islayer}'>
    _c += ".${idname}Mc {display:block;width:100%;position:fixed;"+(hc.position?hc.position:'bottom')+":0;overflow:visible !important;left:0;background:rgba(0,0,0,0);display:block;height:${layerheight}px;}";
    _c += ".${idname}Mc .${idname}txt {position:absolute;right:0;top:0;width:100%;height:${layerheight}px;margin:0;padding:0;border:0;filter:alpha(opacity=0);-moz-opacity:0;opacity:0;}";//</c:if>
    _c += ".${idname}He {margin:0;padding:0;display:block;"+(hc.position?hc.position:'bottom')+":0;width:100%;height:"+height+"px;position:fixed;background:url(${imageurl}) rgba(0,0,0,0) no-repeat center;background-size:100% "+height+"px;overflow:hidden;z-index:2147483648 !important;}";
    _c += ".${idname}He .${idname}txt {position:absolute;right:0;top:0;width:100%;height:"+height+"px;margin:0;padding:0;border:0;filter:alpha(opacity=0);-moz-opacity:0;opacity:0;}";
    _c += ".${idname}Bs {position:absolute; bottom:0; left:0; width:30px; height:15px; overflow:hidden;z-index:2147483647;}";
    _c += ".${idname}Bs img {width:30px; height:15px;float:left;}";

    var _b = "";//<c:if test='${islayer}'>
    _b += "<p class='${idname}Mc'><a href='"+url+"' target='_blank' id='${idname}Mc'><span class='${idname}txt'>${description}</span></a></p>";//</c:if>
    _b += "<p class='${idname}He' id='${idname}He'><label class='${idname}Gb' id='${idname}Gb'><img src='${closeimageurl}' border='0' alt='X'/></label>";
    _b += "<a href='"+url+"' target='_blank' id='${idname}He'><span class='${idname}txt'>${description}</span></a><label class='${idname}Bs'><img src='${adimageurl}' alt=''/></label></p>";
    a.A(_c);
    a.B(_b);
    a.E();
    a.F(doc.getElementById("${idname}He"));
    if(hc.position){if(hc.position=='top'){doc.body.style.paddingTop=height+"px";}else{doc.body.style.paddingBottom = height+"px";}}
